...
/**
 * Sticky bottom CTA for phones: appears once the visitor scrolls past the
 * hero, hides while the final CTA section is on screen, and can be dismissed.
 * Keeps the primary conversion action reachable without scrolling on small
 * screens (44px touch targets, iOS safe-area aware). Desktop never sees it.
 */
export function MobileStickyCta() {
  ...
  return (
    <div
      className="fixed inset-x-0 bottom-0 z-40 border-t border-[#d2d2d7]/70 bg-white/95 px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 shadow-[0_-8px_30px_-18px_rgba(0,0,0,0.25)] backdrop-blur-xl md:hidden"
      role="complementary"
      aria-label={t('mobileCtaLabel')}
    >
      <p className="mb-2 text-center text-[11px] font-light text-[#707070]">{t('mobileCtaHint')}</p>
