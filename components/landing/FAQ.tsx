...
export function FAQ({ locale = 'es' }: { locale?: string }): ReactNode {
  ...
  return <section id="faq" aria-labelledby="faq-heading" className="border-t border-[#d2d2d7] bg-white px-5 py-16 md:py-24"><div className="mx-auto max-w-[720px]"><h2 id="faq-heading" className="text-center text-[34px] font-semibold tracking-tight text-[#1d1d1f]">{locale === 'en' ? 'Frequently asked questions' : 'Preguntas frecuentes'}</h2><div className="mt-8 divide-y divide-[#d2d2d7]">{items.map((item) => <details key={item.question} className="py-5"><summary className="cursor-pointer list-none pr-8 text-[15px] font-medium text-[#1d1d1f] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#0071e3]">{item.question}</summary><p className="mt-3 text-[14px] leading-6 text-[#707070]">{item.answer}</p></details>)}</div></div></section>
}
