...
const QUICK_ACTIONS = [
  { labelKey: 'scrape', href: '/pipeline/scrape', icon: Search, descKey: 'scrapeDesc' },
  { labelKey: 'rank', href: '/pipeline/rank', icon: BarChart3, descKey: 'rankDesc' },
  { labelKey: 'createCv', href: '/pipeline/apply', icon: FileText, descKey: 'createCvDesc' },
  { labelKey: 'prepare', href: '/pipeline/interview', icon: TrendingUp, descKey: 'prepareDesc' },
]
...
export default function Dashboard() {
  ...
  const pipelineProgress = pipeline.length
    ? Math.round((pipeline.filter(s => s.done).length / pipeline.length) * 100)
    : 0
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {t('stepsCompleted', {
                  done: pipeline.filter(s => s.done).length,
          ...
          return (
            ...
            >
              <div className={`mx-auto flex h-8 w-8 items-center justify-center rounded-full ${
                done ? 'bg-emerald-100 text-emerald-600' : 'bg-[#f5f5f7] text-[#858585]'
              }`}>
                {done ? (
                  <CheckCircle2 className="size-4" />
                ) : (
              ...
              <p className={`mt-1.5 text-[11px] font-medium ${
                done ? 'text-emerald-700' : 'text-[#474747]'
              }`}>
